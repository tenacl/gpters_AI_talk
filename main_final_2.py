...
def generate_lecture_page():
    response = requests.post(
        ...
        data=json.dumps({
            "model": "gpt-4o",
            "messages": [
                {"role": "system", "content": "당신은 최고의 에듀테크 회사의 직원입니다. 부드럽고 재치있게 작성하고, 이모지를 사용해. 마크다운 형식을 사용해. 다음은 강의 상세 페이지 생성을 위한 요청입니다."},
                {"role": "user", "content": f"""
                다음 예시를 참고하여 입력된 강의 정보를 바탕으로 새로운 강의 상세 페이지를 작성해 주세요. 부드럽고 재치있게 작성하고, 이모지를 사용해. 
                ...
                AI와 함께라면 누구나 프로페셔널한 홍보 콘텐츠를 만들 수 있습니다. 실전 경험이 풍부한 전문가와 함께 AI 시대의 새로운 마케팅 노하우를 습득하세요!
                """}
            ],
            "temperature": 0.2,
        })
